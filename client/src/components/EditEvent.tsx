...
export function EditEvent(props: EditEventProps) {
  ...
  const [startDate, setStartDate] = useState<Date | undefined>(props.event?.startDate ?? undefined);
  const [endDate, setEndDate] = useState<Date | undefined>(props.event?.endDate ?? undefined);
  const [location, setLocation] = useState(props.event?.location ?? '');

  const handleSubmit = async (formEvent: React.FormEvent<HTMLFormElement>) => {
    formEvent.preventDefault();

    const confirmed = window.confirm(`Are you sure you want to edit "${title}"?`);
    if (confirmed) {
        try {
            const response = await fetch(`${API_URL}/events/${props.event?._id}`, {
                method: 'PUT',
                headers: {
